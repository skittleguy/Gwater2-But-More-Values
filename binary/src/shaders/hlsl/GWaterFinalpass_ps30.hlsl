...
float4 main(const PS_INPUT i) : COLOR {
	...
	float3 compiled_lighting0;	// Reflected lights
	float3 compiled_lighting1;	// Diffuse surface
	...
	// Flashlight lighting (eg. a lamp is casting shadow)
		...
		if (flashlightSpacePosition.z > 0) {
			DoSpecularFlashlight(g_FlashlightPos, i.pos, flashlightSpacePosition, smoothed_normal,
				...
				compiled_lighting1, compiled_lighting0);
		}

		#if OPAQUE // Opaque
			return final_output(COLOR2.xyz * compiled_lighting1 + compiled_lighting0);
		#else 	// Translucent
			return final_output(compiled_lighting0);
		#endif
		...
		PixelShaderDoSpecularLighting(i.pos, smoothed_normal,
					...
					// Outputs
					compiled_lighting0, compiled_lighting1);
	#endif
		...
		return final_output(diffuse + compiled_lighting0);
	#else // Translucent
		float fresnel = fresnel_approx(1.0, IOR, i.view_dir, smoothed_normal, 0.0);
		float3 specular = texCUBE(CUBEMAP, reflect(i.view_dir, smoothed_normal)).xyz * ENV_MAP_SCALE * fresnel + compiled_lighting0;
		...
		return final_output(absorption * refracted + compiled_lighting0 + specular);
	#endif
};