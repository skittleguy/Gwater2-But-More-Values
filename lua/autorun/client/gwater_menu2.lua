...
local options = {
	...
	tab = 1,
	parameter_tab_header = "The Parameter Tab",
	...
	about_tab_header = "The About Tab",
...
local function create_slider(self, text, min, max, decimals, dock)  
	...
	slider:SetValue(gwater2.solver:GetParameter(option))
	slider:SetDecimals(decimals)
	if options[text] then 
		options[text].default = options[text].default or gwater2.solver:GetParameter(option)
	else
	...
	function slider:OnValueChanged(val)
		...
		if option == "radius" then 					-- hack hack hack! radius needs to edit multiple parameters!
			gwater2.solver:SetParameter("surface_tension", 0.01 / val^4)
...
local function create_picker(self, text, dock, size)
	...
	mixer:SetVector(gwater2.color:ToVector()) 
	function mixer:ValueChanged(col)
		gwater2.color = Color(col.r, col.g, col.b)
	end
...
concommand.Add("gwater2_menu2", function()
	...
	function mainFrame:Paint(w, h)
		...
		options.solver:RenderParticles(function(pos)
			local depth = math.max((pos[3] - y - 100) / 100, 1.5)
    ...
    local function parameterTab(tabs)
		...
		local explanation_text = options.parameter_tab_text
		function explanation:Paint(w, h)
			...
			surface.SetDrawColor(255, 255, 255)
			surface.DrawOutlinedRect(0, 0, w, h)
			draw.DrawText(explanation_text, "GWater2Title", 6, 6, Color(0, 0, 0), TEXT_ALIGN_LEFT)
			draw.DrawText(explanation_text, "GWater2Title", 5, 5, Color(187, 245, 255), TEXT_ALIGN_LEFT)
		end

		-- parameters
		local sliders = {}
		create_label(scrollPanel, "Fun Parameters", "These parameters directly influence physics and visuals", 0, 315)
		...
		function scrollPanel:AnimationThink()
			...
			if text_name then
				...
				explanation_text = text_name
			else
				explanation:SetText(options.parameter_tab_text)
				explanation_text = options.parameter_tab_header
			end
		end
